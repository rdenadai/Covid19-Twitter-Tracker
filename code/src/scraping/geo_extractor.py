...
if __name__ == "__main__":
    ...
    usernames = [result.username.replace("@", "") for result in results]
    ...
    with ProcessPoolExecutor(max_workers=cpu_count()) as executor:
        ...
        for i, content in enumerate(contents):
            try:
                ...
                pass
        print(f"--- Data norm took {round(time.time() - start_time, 2)} seconds ---")

        start_time = time.time()
        executor.map(run_save_user_location, norm_geo_users, chunksize=25)
